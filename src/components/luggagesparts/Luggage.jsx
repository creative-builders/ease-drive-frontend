import { useState } from 'react'
import { Link } from "react-router-dom"
import Search from '../../assets/images/search.png'
import locate from '../../assets/images/location-icon.png'
import CustomButton from '../CustomButton';
import images from '../../assets/images/download.png'
import Header from '../../layout/dashboard/header/Header';
import BackArrow from '../BackArrow';


export default function Luggage(){

    const [showLuggageUpload, setShowLuggageUpload] = useState(false);

    const [file, setFile] = useState(null);
    const [previewUrl, setPreviewUrl] = useState('');

    const handleFileChange = (e) => {
        const selectedFile = e.target.files[0];
        if (selectedFile) {
            setFile(selectedFile);

            // Show preview if it's an image
            const fileType = selectedFile.type;
            if (fileType.startsWith('image/')) {
                const reader = new FileReader();
                reader.onloadend = () => {
                    setPreviewUrl(reader.result);
                };
                reader.readAsDataURL(selectedFile);
            } else {
                setPreviewUrl(''); // no preview for non-images
            }
        }
    };

    return(
        <div className='min-h-screen w-full bg-[url(/Group.png)] flex flex-col items-center justify-around gap-14'>
            <Header />
            <BackArrow  extendedStyles='top-24 xl:top-24 left-2 xl:left-20' />
            <article className='h-17 flex w-4/5 xl:w-[667px] px-4 py-6 mt-24 items-center gap-6 rounded-2xl bg-[#fff]'>
                <img src={locate} alt="" />
                <input type="text" className='w-4/5 indent-2 text-xl outline-0 bg-transparent focus:outline-none focus:ring-0 placeholder:text-[#444]' placeholder='UNN 2nd gate' />
            </article>
            <main className='w-full xl:w-[1176px] h-fit flex flex-col bg-[#F8FDF9] p-3 items-center justify-around mt-8 rounded-t-[32px] rounded-r-[32px] rounded-b-none flex-shrink-0'>
               
               <figure className='w-4/5 xl:w-4/5 gap-6 flex flex-col items-start justify-end h-fit'>
                <div className=" h-16 w-3/5 xl:w-72 flex items-center justify-between p-2">
                    <img src={Search} alt="" />
                    <div className=" w-60 flex flex-col gap-2">
                        <h2 className='font-[poppins] text-base not-italic font-medium leading-9'>Current Location</h2>
                        <p className='text-[#B8B8B8] font-[poppins] text-[12px] font-normal not-italic leading-[18px]'>unn 2nd gate</p>
                    </div>
                </div>
                
                <div className=" h-16 w-3/5 xl:w-72 flex items-center justify-between p-2">
                    <img src={locate} alt="" />
                    <div className=" w-60 flex flex-col gap-2">
                        <h2 className='font-[poppins] text-base not-italic font-medium leading-9'>Top</h2>
                        <p className='text-[#B8B8B8] font-[poppins] text-[12px] font-normal not-italic leading-[18px]'>Ogeige market</p>
                    </div>
                </div>
                
                <article className='h-10 w-3/5 xl:w-72 inline-flex items-center justify-start gap-5 p-2 bg-transparent'>
                    <input
                        className='h-5 xl:h-7 w-6 xl:w-8 rounded-none bg-transparent border border-[#20AE3A]'
                        type="radio"
                        name="gender"
                        id="with-luggage"
                        onChange={() => setShowLuggageUpload(false)}
                    />
                    <span className='font-[poppins] text-[12px] xl:text-base font-medium not-italic leading-6'>without luggage</span>
                </article>

                <article className='h-10 w-3/5 xl:w-72 inline-flex items-center justify-start gap-5 p-2 bg-transparent'>
                    <input
                        className='h-5 xl:h-7 w-6 xl:w-8 rounded-none bg-transparent border border-[#20AE3A]'
                        type="radio"
                        name="gender"
                        id="without-luggages"
                        onChange={() => setShowLuggageUpload(true)}
                    />
                    <span className='font-[poppins] text-[12px] xl:text-base font-medium not-italic leading-6'>with luggages</span>
                </article>

                {showLuggageUpload && (
                    <section className='flex flex-col items-left mt-5 h-[280px] w-full gap-6 p-2 relative'>
                        <p className='font-[poppins] text-[#5A5A5A] not-italic text-[20px] leading-6 font-medium capitalize'>
                        upload picture of your luggages
                        </p>

                        <label
                        htmlFor="upload"
                        className='h-[200px] w-full cursor-pointer relative flex-shrink-0 xl:flex items-center justify-center border bg-[#FEFEFE] rounded-lg'
                        >
                        {!file && (
                            <img
                            src={images}
                            className='h-12 w-14 absolute top-[50%] left-[50%] transform -translate-x-1/2 -translate-y-1/2 cursor-pointer'
                            alt="Upload"
                            />
                        )}
                        {previewUrl && (
                            <img
                            src={previewUrl}
                            alt="Preview"
                            className='absolute top-0 left-0 w-full h-full object-top rounded-lg'
                            />
                        )}
                        {file && !previewUrl && (
                            <p className='text-center text-sm text-gray-600 px-4'>
                            {file.name}
                            </p>
                        )}
                        </label>

                        <input
                            type="file"
                            accept=".pdf, .doc, .docx, .xls, .xlsx, .txt, .png, .jpg, .jpeg, .gif, .svg"
                            id="upload"
                            hidden
                            name="picture"
                            onChange={handleFileChange}
                        />
                    </section>
                )}


               <Link to='/LookingFor' className='w-full mb-6'>
                    <CustomButton 
                        name="Contiune"
                        extendedStyles={"p-3 lg:p-4 w-full"}
                        // btnClick={() => nextStep()}
                    />
               </Link>
               </figure>
               
            </main>
        </div>
    );
}